    ...
    def Answer_Weather(self,  NER_dict):
        ...
        if language_weather == 'EN':
            ...
            try:
                ...
                CITY = self.selenium (website)
                self.answer_weather(today,tomorrow,  "в каком городе?", 'когда?', "сегодня", "завтра", CITY,
        ...
        elif language_weather == 'ET':
            try:
                CITY = "_".join(CITY.split())
                website = 'https://et.wikipedia.org/wiki/'+CITY
                CITY = self.selenium (website)
                self.answer_weather(today,tomorrow,  "Millises linnas?", 'Kui', "täna", "homme", CITY,
        ...
        elif language_weather == 'DE':
            try:
                CITY = "_".join(CITY.split())
                website = 'https://de.wikipedia.org/wiki/'+CITY
                CITY = self.selenium (website)
                self.answer_weather(today,tomorrow,  "In welcher stadt?", 'Wenn?', "heute", "morgen", CITY,
                           "Entschuldigung, wir haben keine Informationen über diese Stadt", 'de')   
            except:
                if CITY != None:
                    CITY = translator.translate(CITY, dest='de').text
                    self.answer_weather(today,tomorrow,  "In welcher stadt?", 'Wenn?', "heute", "morgen", CITY,
                           "Entschuldigung, wir haben keine Informationen über diese Stadt", 'de')  
                else:
                    self.answer_weather(today,tomorrow,  "In welcher stadt?", 'Wenn?', "heute", "morgen", CITY,
