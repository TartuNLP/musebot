...
class TravelBot(Answers):
    ...
    def Answer_Travel(self, NER_dict):
        ...
        elif language_travel == 'ET':
            self.answer_travel(CITY, CITY2, leaving, arriving, today, tomorrow, 'et', "BOT: Mis on teie sihtkoht?",
            "Milline linn sooviksite lahkuda?", "BOT: Mul on kahju, et sisestatud linn ei lange kokku sellega, mida sa varem ütlesid",
             "BOT: Millal?", "täna", "homme")
        elif language_travel == 'RU':
            self.answer_travel(CITY, CITY2, leaving, arriving, today, tomorrow, 'ru', "BOT: Куда ты направляешься",
            "из какого города вы хотели бы уехать?", "БОТ: извините, введенный вами город не совпадает с тем, который вы мне сказали ранее",
            "BОТ: Когда?", "Сегодня", "Завтра")  
        
        elif language_travel == 'DE':
            self.answer_travel(CITY, CITY2, leaving, arriving, today, tomorrow, 'de', "BOT: Was ist dein ziel?",
            "Aus welcher Stadt möchten Sie abreisen?", "BOT: Es tut mir leid, die Stadt, die Sie eingegeben haben, stimmt nicht mit der zuvor eingegebenen überein",
            "BОТ: Wenn?", "heute", "morgen")
